...
namespace IocPerformance.Benchmarks.Prepare
{
    ...
    public class PrepareAndRegisterAndSimpleResolve_13_Benchmark : PrepareAndRegister_12_Benchmark
    {
        ...
        {
            container.PrepareBasic();
            container.Resolve(Index.GetDummyOne);
            container.Resolve(Index.GetSingletonOne);
            container.Dispose();
        }

        public override void Verify(Adapters.IContainerAdapter container)
        {
            if (SingletonOne.Instances != this.LoopCount)
            {
                throw new Exception(string.Format("Singleton1 count must be {0} but was {1}", this.LoopCount, SingletonOne.Instances));
            }
        }
    }
}
